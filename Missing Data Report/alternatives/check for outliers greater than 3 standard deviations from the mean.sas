...
libname raw "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\data\Raw";
libname derived "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\data\Derived";
footnote "SAS program stored: Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\programs\DSMB_report_edited 20180423.sas";
...
ods rtf file = "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\summary..&sysdate.doc" style=journal;
...
BMi = 703*weight /(height)**2;                                 /*CREATE BMI in inches and lbs*/
	...
	keep     BMi	activity_walking	age	creatinine	current_smoker	diabetes egfr_value	egfr_non_aa	non_aa_oth	egfr_aa	aa_oth	gender	glucose	height	height_2	height_2_units	height_units	hemoglobin_a1c_value	high_cholesterol	htn	hyperkalemia_serum	insulin	osa	peak_bld_diastolic_1	peak_bld_systolic_1	potassium	prior_smoker	race___1	race___2	race___3	race___4	race___5	race___6	redcap_event_name	site_1	sodium	stan_diastolic_2	stan_systolic_2	study_id	study_site	total_meters_walked	weight	weight_2	weight_2_units	weight_units
;

*At this point we are eliminated all but the baseline data for actual patients only keeps those;
	if REDCAP_EVENT_NAME = "baseline_visit_arm_1" and study_id ne "samp" and study_id not in ("sample","TestMedicationPiping");
run;
...
/*CREATE RTF OF MEANS*/
ods rtf file= "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\documents\output\penn_summary_stat &sysdate..doc" style=journal;
...
/*SUMMARY STATS*/
proc contents data=raw.penn_knockout varnum;run;  /*UNDERSTAND THE SIZE, AND VARIABLE TYPES OF THE DATASET*/
...
/*From: Harnish H Patel [mailto:harnish.patel@northwestern.edu] */
/*Sent: Monday, April 30, 2018 10:36 AM*/
/*To: Chittams, Jesse L. <chittams@nursing.upenn.edu>*/
/*Cc: Rhodes, Joseph M <josephrh@nursing.upenn.edu>*/
/*Subject: Re: Knockout study DSMB Open report*/
/**/
/*Hi Jesse,*/
/**/
/*Here are corrected values:*/
/**/
/*Height: 157 cm */
/**/
/*Best,*/
...
data derived.nw;
	set raw.nw_knockout;
	if study_Id = "KT-2-20" then height_2 = 157;
	study_site = 2;
...
ods rtf file= "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\documents\output\nw_summary_stat &sysdate..doc" style=journal;
...
var study_id egfr_value	egfr_non_aa	non_aa_oth	egfr_aa	aa_oth;
run;
...
data derived.demo;
	...
	egfr_valuen = egfr_value*1;
	if egfr_value="" then egfr_valuec=. ;
	else if egfr_valuen ge 30 and egfr_valuen le 45 then egfr_valuec=1;
	else if egfr_valuen gt 45 and egfr_valuen le 60 then egfr_valuec=2;
    else if egfr_valuen gt 60 or egfr_valuen=. then egfr_valuec =3;
	format egfr_valuec egfr.;
	if trt ne .;
run;


*QA for egfr_value to numeric egfr_valuen;
	
 proc means data=derived.demo;
 var egfr_valuen;
 run;

*QA for new categorical variable egfr_valuec (use this for large data sets);
	

proc freq data=derived.demo;
tables egfr_valuec*egfr_value/list missing ;
run;

*QA for new categorical variable egfr_valuec;


proc print data=derived.demo;
 var study_id egfr_value egfr_valuen egfr_valuec;
run;
...
where race = 0;
var  study_id study_site race trt; 
run;
...
proc print data=demo1;
where . < bmi < 18;
var  study_id study_site bmi BMi weight height; 
run;
...
%include 'Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\programs\draft\macro_table1.sas';

%cont(total_meters_walked,%str(Total Meters Walked), 1,demo1);
%cont(Age,%str(Age,years), 3,demo1);
...
%catg(egfr_valuec,EGFR,egfr.,fisher,20,%str(where egfr_valuec ne .),demo1);
...
ods rtf file=  "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\documents\output\closed_report_table_1 &sysdate..doc" style=journal;
...
*ods rtf file= "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\documents\output\open_report_table_1_cont &sysdate..doc" style=journal;


ods rtf file= "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\documents\output\open_report_table_1 &sysdate..doc" style=journal;
...
ods rtf file= "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\documents\output\egfr_variable summary stats &sysdate..doc" style=journal;
...
ods rtf file= "Q:\Julio Chirinos\Knock_out_Summer_2016\DSMB\DSMB_spring_2018\documents\output\DSMB QA MEANS+FREQ &sysdate..doc" style=journal;
...
*email from Julio 20171110;
*Hi Jesse,
I would     egfr_value   stratify as:
- >60
- 45-60
- 30-45
Thank you!

Julio A. Chirinos, MD, PhD, FAHA, FESC;
