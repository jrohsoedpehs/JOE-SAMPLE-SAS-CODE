/*Regression on two continuous variables*/
...
/*Regression*/
proc mixed data=derived.DFH_Data_Fall_2018_anal method=ml; 
	class id; 
	model midheartratewk=postpedometerreadingwk/s cl;
	random intercept /type=un subject=id;
	where agegp=1;
	title "Mixed Model for Mid-Dance HR Regressed on Pedometer Steps";  
run;
