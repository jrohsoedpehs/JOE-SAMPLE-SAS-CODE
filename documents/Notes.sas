/*libname notes "";*/
footnote "SAS Program stored in: U:\Jesse Chittams\admin\Joseph Rhodes\Notes\SAS\notes";
/***
...
CREATE A FOOTNOTE
FOOTNOTE "SAS PROGRAM STORED: DIRECTORY PATH\TITLE.SAS";
...
data raw.checkmissing injnotd indnotj;
	merge raw.CuidalosDanielle586n(in=d) raw.CuidalosJoe699n(in=j); /*in makes d and j dummy variables. d = 1 if the data is in the indnotj (Danielle's) data set and vice versa*/
  	by staff_id year month day;
	if d = 1 and j ne 1 then output indnotj;/*Danielle entered, but Joe didn't*/
	if j = 1 and d ne 1 then output injnotd; /*Joe entered, but Danielle didn't*/
run;
