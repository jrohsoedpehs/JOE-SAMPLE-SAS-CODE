...
/*%QAreverse(dsn,vars);*/
/*%QASUMscore(dsn,name,scoreVars);*/
/*%QAMEANscore(dsn,name,scoreVars);*/
...
/*QA scores*/
%macro QASUMscore(dsn,name,scoreVars);
proc freq data=&dsn;
tables &name._nmiss * &name._pctmiss * &name._score * &scoreVars /list missing;
run;
%mend;

%macro QAMEANscore(dsn,name,scoreVars);
proc freq data=&dsn;
tables &name._score * &scoreVars /list missing;
run;
%mend;
