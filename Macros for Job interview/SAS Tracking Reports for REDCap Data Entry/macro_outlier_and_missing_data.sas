...
data temp;
    set &mergedsn;
    keep caregiver_id redcap_event_name &FORMVARS;
run;
...
proc sql noprint;   
    ...
    separated by ' '   
    from cont_out   
    where type = 2 and name notin("caregiver_id","redcap_event_name");
/*	char type=2*/
/*	num  type=1*/
/*	BEST IS THE DEFAULT NUM FORMAT, USUALLY IS NUMERIC*/
quit; 
...
data &dsn;
    set &mergedsn;
	keep caregiver_id redcap_event_name site &reportVars;
	if &subset=.;
