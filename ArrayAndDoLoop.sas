%macro freq(dataSetName, variable);
proc freq data=  &dataSetName;
   tables &variable ;
run;     
%mend;
/*%freq(dataSetName, variable)*/



 %freq(raw.f1, SCREENING_YN)
...
data test1;
   set dsn;
   NE_I5A_HT_c=compress(NE_I5A_HT,"-ABCDEFGHIJKLMNOPQRSTUVWXYZ<>%",);
run;

proc freq data = test1;
tables NE_I5A_HT_c*NE_I5A_HT/list missing;
run;
