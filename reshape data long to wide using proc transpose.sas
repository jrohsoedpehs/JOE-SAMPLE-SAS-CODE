...
2. Transposing two variables
 ...
 With only a few modifications, theabove example can be used to reshape two (or more) variables. The approach here is to use proc transpose multiple times as needed. Themultiple transposed data files then are merged back.  
...
Sometimes, there is no variable in the dataset that uniquely identifies each observation.  Rather, two or more variables arenecessary to uniquely identify each observation.  In this situation, wehave to specify these variables in the by statement.
...
The following example is a more realisticexample that uses a data file having 300 records in long format (50 wide records andsix time points).
...
The following example shows how to reshapemultiple variables, some of which are numeric and other that are character (i.e., string) variables. The approach here is the same as inExample 2 that proc transpose is used multiple times and the data filesare then merged together.  
